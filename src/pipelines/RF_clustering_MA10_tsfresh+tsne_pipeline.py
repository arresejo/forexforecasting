...
if __name__ == '__main__':
    ...
    reduce_features_tsne.reduce_features(feature_method=TSFRESH, ma_windows_size=MA_WINDOW_SIZE)

    hdbs.run_model(TSFRESH_TSNE, ma_windows_size=MA_WINDOW_SIZE, plotting=False)

    build_features_prediction_with_clustering.build_features(feature_method=TSFRESH_TSNE, ma_windows_size=MA_WINDOW_SIZE)

    RF_clustering_model.run_model(feature_method=TSFRESH_TSNE, ma_windows_size=MA_WINDOW_SIZE, plotting=False)
